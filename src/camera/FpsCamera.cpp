...
void FpsCamera::SetViewport(float aspectRatio)
{
...
glm::mat4 FpsCamera::ProjectionMatrix() const
{
...
void FpsCamera::SetYawPitch(float yawRadians, float pitchRadians)
{
    m_Yaw = yawRadians;
    m_Pitch = pitchRadians;
}

void FpsCamera::SetFieldOfView(float fieldOfViewRadians)
{
    m_FieldOfView = fieldOfViewRadians;
}

void FpsCamera::SetNearFar(float nearPlane, float farPlane)
{
...
void FpsCamera::SetMouseSensitivity(float radiansPerPixel)
{
    m_MouseSensitivity = radiansPerPixel;
}

void FpsCamera::Update(const InputState& input, float deltaSeconds)
{
    glm::vec3 forward = Forward();
